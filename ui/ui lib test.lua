...
-- name, function that returns string, size in UDim2, sizex, sizey, looped, transparency, default state(minmalizeed/opened)
spawn(function()
...
function addDrag(a)
	local b = game:GetService("Players").LocalPlayer:GetMouse()
	...
	if e then
		a.Active = true
		f:connect(function()
			local g = a.InputBegan:connect(function(h)
				if h.UserInputType == Enum.UserInputType.MouseButton1 then
					local i = Vector2.new(b.X - a.AbsolutePosition.X, b.Y - a.AbsolutePosition.Y)
					while d:wait() and c:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) do
						pcall(function()
							a:TweenPosition(UDim2.new(0, b.X - i.X, 0, b.Y - i.Y), "Out", "Linear", 0.1, true)
						end)
					end
				end
			end)
			local j
			j = a.MouseLeave:connect(function()
				g:disconnect()
				j:disconnect()
			end)
		end)
...
function library:CreateWindow(nazwa, winPosition) -- nazwa rozmiar pozycja
	...
	function library:CreateTextbox(name, textboxtext, textpos, sizex, sizey, looped, transp, toggled)
		sizex = sizex or 200
		sizey = sizey or 300
